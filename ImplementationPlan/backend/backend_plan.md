...
engine.  The backend is a **Python 3.11+ / FastAPI server** that owns all
simulation state and streams results to the frontend via WebSocket.  REST
endpoints handle configuration, presets and body manipulation.

### 1. Define constants and types

1. Create the vector and body types described in `data_structures.md`.
   Use NumPy arrays for vector operations and Pydantic models for API
   schemas.
...
1. Write a function `compute_accelerations(state: SimulationState)` that
   ...
   【658212011942306†L176-L184】.  Leverage NumPy vectorised operations for
   pairwise force summation.
...
1. **Velocity Verlet:**  Implement the leapfrog update in the form
   ...
   `compute_accelerations` to obtain new accelerations.
...
### 7. Server architecture (FastAPI)

1. Create a FastAPI application that hosts the simulation engine.  Run a
   single `SimulationEngine` instance (single‑user, one simulation at a
   time).
2. Implement a **WebSocket endpoint** (`/ws/simulation`) for real‑time
   streaming.  When the simulation is running, the server pushes state
   snapshots (body positions, velocities, trails, diagnostics) at ~60 FPS.
   The WebSocket also receives control commands from the client:
   `start`, `pause`, `step`, and `set_speed`.
3. Use an **asyncio background task** for the simulation loop.  On each
   tick, call the `step()` function, serialise the snapshot and broadcast
   it over the WebSocket connection.
4. Implement **REST endpoints** for configuration and state management
   (see section 8).
5. Serve the built frontend (Vite production build) as static files from
   the FastAPI app in production.  During development, run Vite on `:5173`
   and FastAPI on `:8000` separately.

### 8. API design for the frontend

Expose REST and WebSocket endpoints for the UI to interact with the
simulation:

**REST endpoints:**

```
POST   /api/init              — Initialise simulation with bodies and params
POST   /api/reset             — Reset to initial conditions
PUT    /api/config            — Update simulation parameters (integrator, dt, ε, chaos mode)
PUT    /api/body/{id}         — Modify a specific body (mass, position, velocity)
POST   /api/body              — Add a new body
DELETE /api/body/{id}         — Remove a body
GET    /api/presets           — List available presets
GET    /api/presets/{name}    — Load a specific preset's initial conditions
```
...
Server → Client messages (JSON):
  {
    "type": "snapshot",
    "time": 12.345,
    "bodies": [ { "id": "...", "position": [x, y], "velocity": [vx, vy], "trail": [...] } ],
    "diagnostics": { "total_energy": ..., "energy_drift": ..., ... }
  }
```

By separating the physics logic from the rendering, this architecture makes
the backend independently testable with pytest.  The server can run long
trajectories without browser constraints, and the frontend interpolates
